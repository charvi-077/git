			 ...
			 enum todo_item_flags flag)
{
	...
	strbuf_addf(buf, "\n%c ", comment_line_char);
	strbuf_addf(buf, nth_commit_msg_fmt, ++opts->current_fixup_count + 1);
	strbuf_addstr(buf, "\n\n");
	...
	if (((flag & TODO_AMEND_MSG) || (flag & TODO_EDIT_AMEND_MSG)) && !seen_squash(opts)) {
		/*
				  ...
				  enum todo_item_flags flag)
{
	...
	if (opts->current_fixup_count > 0) {
		...
		strbuf_addf(&header, combined_commit_msg_str,
			    opts->current_fixup_count + 2);
		strbuf_splice(&buf, 0, eol - buf.buf, header.buf, header.len);
		strbuf_release(&header);
		if (((flag & TODO_AMEND_MSG) ||
		     (flag & TODO_EDIT_AMEND_MSG)) &&
		     !seen_squash(opts))
		...
		strbuf_addf(&buf, combined_commit_msg_str, 2);
		strbuf_addf(&buf, "\n%c ", comment_line_char);
		strbuf_addstr(&buf, (flag & TODO_AMEND_MSG) ?
			      skip_first_commit_msg_str :
			      first_commit_msg_str);
		strbuf_addstr(&buf, "\n\n");
		if ((flag & TODO_AMEND_MSG) || (flag & TODO_EDIT_AMEND_MSG))
	...
	if (command == TODO_SQUASH || (flag & TODO_AMEND_MSG) ||
		...
		strbuf_addf(&buf, "\n%c ", comment_line_char);
		strbuf_addf(&buf, skip_nth_commit_msg_fmt,
		...
		*commit_todo_item_at(&commit_todo, item->commit) = item;
		...
		if (i2 >= 0) {
			...
			if (starts_with(subject, "amend!")) {
				...
				todo_list->items[i].flags = TODO_AMEND_MSG;
