...
test_auto_squash () {
	...
	git add -u &&
	test_tick &&
	git commit -m "squash! first" -m "message body for first" &&
	git tag $1 &&
	test_tick &&
	git rebase $2 -i HEAD^^^ &&
	git log --oneline >actual &&
...
test_expect_success 'auto squash that matches a sha1' '
	...
	git commit -m "squash! $oid" -m "message body" &&
	...
	test_cmp expect actual &&
	git cat-file commit HEAD^ >commit &&
	grep "^message body" commit &&
	grep "message body" commit >actual &&
	test_line_count = 1 actual
'
...
test_auto_commit_flags () {
	...
	git add -u &&
	test_tick &&
	git commit --$1 first-commit -m "message body for first" &&
...
test_auto_fixup_fixup () {
	...
	git add -u &&
	test_tick &&
	git commit -m "$1! first" -m "message body for first" &&
	echo 2 >file1 &&
	git add -u &&
	test_tick &&
	git commit -m "$1! $2! first" -m "second message body for first" &&
	git tag "final-$1-$2" &&
	test_tick &&
	(
		set_cat_todo_editor &&
...
test_expect_success C_LOCALE_OUTPUT 'autosquash with custom inst format' '
	...
	test_tick &&
	oid=$(git rev-parse --short HEAD^) &&
	git commit -m "squash! $oid" -m "message body for first" &&
	echo 1 >file1 &&
	git add -u &&
	test_tick &&
	subject=$(git log -n 1 --format=%s HEAD~2) &&
	git commit -m "squash! $subject" -m "second message body for first" &&
