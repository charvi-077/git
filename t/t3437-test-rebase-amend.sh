...
'
test_expect_success 'sequence of amend squash amend with --signoff, only first add SOB' '
	test_when_finished "test_might_fail git rebase --abort" &&
	git checkout --detach branch && 
	FAKE_LINES="1 fixup 2 amend 3 amend 4 squash 5 amend 6" \
		FAKE_MESSAGE_COPY=commented-msg \
			git rebase -i A --signoff  
'
test_expect_success 'first amend commented out in sequence fixup amend amend' '
	...
	git cat-file commit HEAD | sed '1,7d' >expected-commit-message && 
	FAKE_LINES="1 fixup 2 amend 3 amend 4" git rebase -i A && 
	git cat-file commit HEAD | sed '1,5d' >actual-commit-message && 
	##FAKE_MESSAGE_COPY=msg git commit --amend && 
	test_cmp_rev HEAD^ A &&
