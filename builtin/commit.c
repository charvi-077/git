...
static int prepare_amend_commit(struct commit *commit, struct strbuf *sb,
	...
	/*
	 * If we amend the 'amend!' commit then we don't want the
				      ...
				      struct wt_status *s)
{
	...
	if (fixup_message) {
		...
		if (fixup_commit && !strncmp(fixup_commit, ":", 1) &&
...
int cmd_commit(int argc, const char **argv, const char *prefix)
{
	...
	static struct option builtin_commit_options[] = {
		 ...
		 * Here amend is an option to the --fixup, that creats
		 * amend! commit.
		 */
